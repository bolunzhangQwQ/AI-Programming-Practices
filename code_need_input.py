...
import pandas as pd
#输入起止点经纬度
start_lon=input("请输入出发点经度")
start_lat=input("请输入出发点纬度")
end_lon=input("请输入终点经度")
end_lat=input("请输入终点纬度")
#输入出发时间
time_choose=input("输入出发时间:输入0-7:00输入1-7:10输入2-7:20输入3-7:30输入4-7:40输入5-7:50输入6-8:00...输入17-9:50")#为便于测试，减少时间成本，时间范围规定在7：00到9：50
G = ox.graph_from_address('上海交通大学闵行校区', dist=10000, network_type='all')  # 获取道路数据，测试暂定交大附近10km
ox.plot_graph(G,bgcolor='white',node_color='blue',edge_color='grey',node_size=5,show=True)

origin_point = (start_lat,start_lon)  # 出发点经纬度
destination_point = (end_lat,end_lon)  # 终点经纬度
